...
**Apply is not in CI.** Apply runs from a developer's authenticated workstation or from a dedicated supervised "apply runner" (a small EC2 instance or ECS task with elevated IAM, accessible only via SSO + MFA). Procedure: assume the apply role via SSO, run `terraform plan` first as a final sanity check (re-verifies against current state), then `terraform apply -auto-approve` on the plan file. This is the deliberate human pause that protects EBS / EKS / IAM from auto-applied error.
