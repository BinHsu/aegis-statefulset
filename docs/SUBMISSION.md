...
| **Pre-commit hook with 4 checks** (anonymisation / Wave 1 stale / credential leak / kubeconform) | Adds 5–10 sec to every commit. Mitigated by the hook being skippable for emergency commits via `--no-verify` (with the operator explicitly accepting the risk). |
