...
| **Terraform state** | Remote state in S3 with native S3 locking (`use_lockfile`, terraform 1.10+) | S3 versioning + KMS encryption | CloudTrail records every state put/get | State file may contain plaintext secrets — rotated periodically; sensitive() declarations in code | S3 conditional writes (`If-None-Match`) prevent concurrent apply | IAM least-privilege on state bucket |
