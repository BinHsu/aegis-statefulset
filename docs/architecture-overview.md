...
- **DynamoDB Global Tables placement table (ADR-03).** Six-property contract: atomic CAS / strong reads / multi-AZ durable / low latency / CDC / audit log. Customer can substitute any backend that satisfies the contract; DynamoDB is the POC reference.
...
- **OpenTelemetry-only instrumentation (ADR-06).** Vendor-neutral at the instrumentation layer; backend (Grafana Cloud / Amazon-managed Prometheus + Grafana) reversible.
- **Defense in depth (ADR-07).** Edge TLS → NetworkPolicy → PSS restricted → Cosign admission → ESO + IRSA → per-tier KMS → eBPF runtime → tamper-protected audit. Each layer names the threat the layer above can't see.
...
- **SHA pin + SLSA L3 + SBOM (ADR-09).** Five lines of YAML buy "the image you reviewed is the image that runs." Audit-grade chain of custody from source to running container.
- **FinOps as architecture, not afterthought (ADR-10).** Tagging discipline + AWS Budgets + Cost Anomaly Detector + per-tenant CUR + Athena attribution + Savings Plans strategy — all day-1, not bolted on.
